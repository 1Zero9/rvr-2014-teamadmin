...
export default async function FundPage() {
  ...
  return (
    <PortalPage
      member={member}
      active="/fund"
      eyebrow="TRANSPARENT TEAM ACCOUNTS"
      title="Team fund"
    >
      <div className="metric-row">
        <div className="metric">
          <span>Available balance</span>
          <strong>€{((income - expense) / 100).toFixed(2)}</strong>
        </div>
        <div className="metric">
      ...
      <article className="panel">
        <div className="section-heading">
          <div>
            ...
            <h3>Every approved movement of team money</h3>
          </div>
        </div>
        {rows.length === 0 ? (
          <div className="empty-state">
            ...
            <p>The ledger will update as contributions and expenses are recorded.</p>
          </div>
        ) : (
              ...
              {rows.map((r) => (
                <tr key={r.id}>
                  ...
                  <td>
                    ...
                    {member.role !== 'parent' && (
                      <small>
                        {r.personName} · {r.paymentMethod || 'method not recorded'}
                      </small>
                    )}
                  </td>
                  <td>{r.category}</td>
                  ...
                  <td className={r.type === 'income' ? 'money-in' : 'money-out'}>
                    {r.type === 'income' ? '+' : '−'}€{(r.amountCents / 100).toFixed(2)}
                  </td>
                </tr>
              ))}
            </tbody>
