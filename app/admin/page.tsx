...
export default async function AdminPage() {
  ...
  let people: typeof members.$inferSelect[] = [];
  let history: typeof auditLog.$inferSelect[] = [];

  try {
    const [pList, hList] = await Promise.all([
      getDb().select().from(members).orderBy(members.displayName),
      getDb().select().from(auditLog).orderBy(desc(auditLog.createdAt)).limit(20),
    ]);
    people = pList;
    history = hList;
  } catch (err) {
    ...
    >
      <div className="metric-row">
        <div className="metric">
          ...
          <strong>{people.filter((p) => p.approved).length}</strong>
        </div>
        <div className="metric">
      ...
      <div className="page-grid">
            ...
            <tbody>
              {people.map((person) => (
                <tr key={person.id}>
                  ...
                  <td>
                    <span
                      className={`badge ${person.approved ? 'paid' : 'pending'}`}
                    >
                      {person.approved
                  ...
                  <td>
                    {member.role === 'super_admin' ? (
                      <form className="role-form" action={updateMember}>
                        ...
                        <select
                          name="approved"
                          defaultValue={String(person.approved)}
                        >
        ...
        <article className="panel">
          ...
          {history.length === 0 ? (
            <div className="empty-state">
              <ShieldCheck />
              <strong>No changes logged yet</strong>
            </div>
          ) : (
              ...
              {history.map((item) => (
                <div className="idea-card" key={item.id}>
                  ...
                  <div className="idea-meta">
                    ...
                    <time>{new Date(item.createdAt).toLocaleString('en-IE')}</time>
                  </div>
                </div>
              ))}
            </div>
